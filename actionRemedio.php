    ...
    <div class="container text-center mb-3 mt-3" style="padding-top: 300px; padding-bottom: 300px;">
        ...
        <?php
            ...
            if($_SERVER["REQUEST_METHOD"] == "POST"){
                ...
                $fotoRemedio = $nomeRemedio = $descricaoRemedio = "";

                //Variável booleana para controle de erros de preenchimento
                $erroPreenchimento = false;

                //Validação do campo nomeRemedio
                //Utiliza a função empty() para verificar se o campo está vazio
                if(empty($_POST["nomeRemedio"])){
                    echo "<div class='alert alert-warning text-center'>O campo <strong>NOME</strong> é obrigatório!</div>";
                    $erroPreenchimento = true;
                }
                else{
                    //Armazena valor do formulário na variável
                    $nomeRemedio = filtrar_entrada($_POST["nomeRemedio"]);    
                }

                //Validação do campo telefoneRemedio
                //Utiliza a função empty() para verificar se o campo está vazio
                if(empty($_POST["descricaoRemedio"])){
                    echo "<div class='alert alert-warning text-center'>O campo <strong>DESCRIÇÃO</strong> é obrigatório!</div>";
                    $erroPreenchimento = true;
                }
                else{
                    //Armazena valor do formulário na variável
                    $descricaoRemedio = filtrar_entrada($_POST["descricaoRemedio"]);
                }
                //Início da validação da foto do usuário
                $diretorio    = "img/"; //Define para qual diretório as imagens serão movidas
                $fotoRemedio  = $diretorio . basename($_FILES['fotoRemedio']['name']); //img/joaozinho.jpg
                $tipoDaImagem = strtolower(pathinfo($fotoRemedio, PATHINFO_EXTENSION)); //Pega o tipo do arquivo em letras minúsculas
                $erroUpload   = false; //Variável para controle do upload da foto

                //Verifica se o tamanho do arquivo é DIFERENTE DE ZERO
                if($_FILES['fotoRemedio']['size'] != 0){

                    //Verifica se o tamanho do arquivo é maior do que 5 MegaBytes (MB) - Medida em Bytes
                    if($_FILES['fotoRemedio']['size'] > 5000000){
                    ...
                    if(!move_uploaded_file($_FILES['fotoRemedio']['tmp_name'], $fotoRemedio)){
                ...
                if(!$erroPreenchimento && !$erroUpload){
                    ...
                    //Cria uma variável para armazenar a QUERY para realizar a inserção dos dados do Usuário na tabela Remedios
                    $inserirRemedio = "INSERT INTO Remedios (fotoRemedio, nomeRemedio, descricaoRemedio) VALUES ('$fotoRemedio', '$nomeRemedio', '$descricaoRemedio')";

                    //Inclui o arquivo de conexão com o Banco de Dados
                    include("conexaoBD.php");

                    //Se conseguir executar a query para inserção, exibe alerta de sucesso e a tabela com os dados informados
                    if(mysqli_query($conn, $inserirRemedio)){

                        echo "<div class='alert alert-success text-center'><strong>Remedio</strong> cadastrado(a) com sucesso!</div>";
                        echo "
                            <div class='container mt-3'>
                                <div class='container mt-3 text-center'>
                                    <img src='$fotoRemedio' style='width:150px;' title='Foto de $nomeRemedio'>
                                </div>
                                <table class='table'>
                                    <tr>
                                        <th>NOME</th>
                ...
                //Redireciona o usuário para o formRemedio.php
                header("location:formRemedio.php");
            }
