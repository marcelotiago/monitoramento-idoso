            ...
            if($_SERVER["REQUEST_METHOD"] == "POST"){
                ...
                if(!$erroPreenchimento){
                    ...
                    if(mysqli_query($conn, $inserirMedicacao)){
                        ...
                        $queryIdoso = "SELECT nomeIdoso FROM Idosos WHERE idIdoso = '$idIdoso' LIMIT 1";
                        $resIdoso = mysqli_query($conn, $queryIdoso);
                        $nomeIdoso = ($row = mysqli_fetch_assoc($resIdoso)) ? $row['nomeIdoso'] : $idIdoso;

                        // Buscar nome do remédio
                        $queryRemedio = "SELECT nomeRemedio FROM Remedios WHERE idRemedio = '$idRemedio' LIMIT 1";
                        $resRemedio = mysqli_query($conn, $queryRemedio);
                        $nomeRemedio = ($row = mysqli_fetch_assoc($resRemedio)) ? $row['nomeRemedio'] : $idRemedio;
